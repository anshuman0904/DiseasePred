...
const originalButtonText = speechBtn.textContent;

speechBtn.addEventListener("click", () => {
  if (isListening) {
    // If already listening, stop recognition
    window.speechSynthesis.cancel();
    speechBtn.textContent = originalButtonText;
    isListening = false;
    return;
  }
  ...
  recognition.onresult = (event) => {
    symptomsInput.value = event.results[0][0].transcript;
    // Process the speech input automatically
    speechBtn.textContent = originalButtonText;
    isListening = false;
  };
...
predictBtn.addEventListener("click", async () => {
  ...
  topPredictionsOutput.innerHTML =
    '<li class="placeholder">Processing your symptoms...</li>';
  ...
  try {
    ...
    if (alternativePredictions.length > 0) {
      alternativePredictions.forEach((prediction) => {
        ...
        li.textContent = `${prediction.disease} - Confidence: ${(
    ...
    speakText(
      `Primary diagnosis is ${diseaseText}. Confidence level is ${confidenceText}`,
